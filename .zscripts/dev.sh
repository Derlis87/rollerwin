...
# Try standalone server first (production build), fallback to dev server
if [ -f "/home/z/my-project/.next/standalone/server.js" ]; then
    echo "[BUN] Found standalone build, starting production server..."
    (
        cd /home/z/my-project
        cp -r .next/static .next/standalone/.next/ 2>/dev/null || true
        cp -r public .next/standalone/ 2>/dev/null || true
        exec node .next/standalone/server.js -p 3000
    ) > /home/z/my-project/.zscripts/server.log 2>&1 &
    DEV_PID=$!
    echo "[BUN] Started standalone server (PID: $DEV_PID)"
else
    bun run dev &
    DEV_PID=$!
    echo "[BUN] Started dev server (PID: $DEV_PID)"
fi
