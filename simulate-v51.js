// Simulation script: Run v5.1 prediction engine against every number in the sequence
// Tracks peaks (consecutive wrong predictions) and identifies failure patterns
...
// ── Import the prediction engine (compiled) ──
// We'll inline the v5.1 color prediction logic directly for simulation
// since we need to simulate per-number, not use the React component
...
const BREAK_PROBS = {
  2: 49.7, 3: 51.8, 4: 51.4, 5: 54.9,
  6: 48.5, 7: 45.3, 8: 44.8
};

function getBreakProb(streakLen) {
  if (streakLen >= 9) return 37.5;
  return BREAK_PROBS[streakLen] ?? 50.0;
}

function simulatePrediction(nums, betType) {
  if (nums.length < 5) return { predicted: null, confidence: 0, mode: 'none' };

  const nonZero = nums.filter(n => n !== 0);
  if (betType === 'color') {
    const cats = ['red', 'black'];
    const getCat = (n) => { const c = getNumberColor(n); return c === 'green' ? null : c };
    ...
    let wheelCat = null, wheelReliability = 0;
    {
      const displacements = [];
      ...
      if (displacements.length >= 3) {
        ...
        if (variance < 12) {
          ...
          if (lastIdx !== undefined) {
            ...
            wheelReliability = Math.floor(Math.max(50, 100 - variance * 4));
            wheelCat = getCat(targetNumber);
          }
        }
      }
    }

    // Markov-2 (recency-weighted for SOFT mode)
    const markov = (() => {
      const trans = {};
      ...
      if (scores.red === 0 && scores.black === 0 && recentSlice.length >= 1) {
    ...
    // Markov-3 (recency-weighted for SOFT mode)
    const markov3 = (() => {
      if (recentSlice.length < 10) return { red: 0, black: 0 };
      const trans = {};
    ...
    const toConfidence = (scores) => {
      const minScore = Math.min(scores.red, scores.black);
      const FLOOR = 5;
      const shifted = {
        red: Math.max(1, scores.red - minScore + FLOOR),
        black: Math.max(1, scores.black - minScore + FLOOR)
      };
      const totalShifted = shifted.red + shifted.black;
      const confs = {};
      cats.forEach(c => {
        const weight = shifted[c] / totalShifted;
        const conf = 48.6 + (weight * 2 - 1) * 30;
        confs[c] = Math.max(5, Math.min(92, conf));
      });
      const sum = confs.red + confs.black;
    ...
    if (currentStreak >= 6) {
      ...
      if (wheelCat && wheelReliability > 60) {
        if (wheelCat === streakColor) {
          scores[wheelCat] += Math.min(20, wheelReliability * 0.3);
        } else if (wheelReliability > 75) {
          scores[wheelCat] += Math.min(10, (wheelReliability - 60) * 0.5);
        }
      }

      const confs = toConfidence(scores);
      const sorted = ['red', 'black'].sort((a, b) => confs[b] - confs[a]);
      return { predicted: sorted[0], confidence: confs[sorted[0]], mode: `ULTRA(streak=${currentStreak})` };
    }

    // ── SOFT mode (streak 2-5) ──
    if (currentStreak >= 2 && currentStreak <= 5) {
      const scores = { red: 0, black: 0 };

      // Markov-2
      const markovWeight = 2.5;
      cats.forEach(c => { scores[c] += markov[c] * markovWeight * 0.2; });

      // Markov-3
      const m3max = Math.max(markov3.red, markov3.black);
      if (m3max > 0) {
        cats.forEach(c => { scores[c] += markov3[c] * 1.8 * 0.3; });
      }

      // Wheel (aligned with break-prob direction for streak 3-5)
      if (wheelCat && wheelReliability > 55) {
        if (currentStreak >= 3) {
          const bp = getBreakProb(currentStreak);
          if (bp > 50 && wheelCat === oppositeColor) {
            scores[wheelCat] += Math.min(15, wheelReliability * 0.3);
          } else if (bp <= 50 && wheelCat === streakColor) {
            scores[wheelCat] += Math.min(15, wheelReliability * 0.3);
          } else if (wheelReliability > 70) {
            scores[wheelCat] += Math.min(8, (wheelReliability - 60) * 0.3);
          }
        } else {
          scores[wheelCat] += Math.min(18, wheelReliability * 0.35);
        }
      }

      // Break-prob nudge (streak 3-5 only)
      if (currentStreak >= 3) {
        const bp = getBreakProb(currentStreak);
        if (bp > 50) {
          const edge = bp - 50;
      ...
      const confs = toConfidence(scores);
      const sorted = ['red', 'black'].sort((a, b) => confs[b] - confs[a]);
      return { predicted: sorted[0], confidence: confs[sorted[0]], mode: `SOFT(streak=${currentStreak})` };
    }

    // ── NORMAL mode (streak < 2) ──
    const freq = {};
    cats.forEach(c => freq[c] = 0);
    const last15 = nonZero.slice(-15);
    last15.forEach(n => { const c = getCat(n); if (c) freq[c]++; });
    const scores = { red: 0, black: 0 };
    cats.forEach(c => { scores[c] += freq[c] * 1.0; });

    // Markov-2 (full history for NORMAL mode)
    const markovFull = (() => {
      const trans = {};
    ...
    cats.forEach(c => { scores[c] += markovFull[c] * 2.5 * 1.2; });

    // Streak analysis (soft reversion at streak=1)
    const streaks = { red: maxR, black: maxB };
    if (streaks.red >= 3) {
      scores.black += 35;
      scores.red -= 35;
    } else if (streaks.red === 2) {
      scores.black += 6;
      scores.red -= 6;
    }
    if (streaks.black >= 3) {
    ...
    cats.forEach(c => {
      ...
      if (count >= 5) {
        const opp = cats.find(x => x !== c);
        scores[opp] += (count - 4) * 10;
        scores[c] -= (count - 4) * 8;
      }
    });
...
for (let i = 5; i < nums.length; i++) {
  ...
  const streakLen = Math.max(maxR, maxB);

  if (!streakStats[streakLen]) streakStats[streakLen] = { correct: 0, wrong: 0, mode: mode };

  const correct = predicted === actualColor;

  if (correct) {
    totalCorrect++;
    if (modeStats[mode]) modeStats[mode].correct++;
    if (streakStats[streakLen]) streakStats[streakLen].correct++;
  } else {
    totalWrong++;
    if (modeStats[mode]) modeStats[mode].wrong++;
    if (streakStats[streakLen]) streakStats[streakLen].wrong++;
  }

  // Peak tracking
  if (correct) {
    if (currentPeakErrors.length > 0) {
      peakHistory.push({
        height: currentPeakErrors.length,
        startIndex: peakStartIdx,
        predicted: currentPeakErrors.map(e => e.predicted),
        actual: currentPeakErrors.map(e => e.actual),
        modes: currentPeakErrors.map(e => e.mode),
        colors: currentPeakErrors.map(e => e.color),
      });
    }
    currentPeakErrors = [];
  } else {
    if (currentPeakErrors.length === 0) peakStartIdx = i;
    currentPeakErrors.push({
      predicted,
      actual: actualColor,
      number: actual,
      mode,
...
// Don't forget the last peak if still open
if (currentPeakErrors.length > 0) {
  peakHistory.push({
    height: currentPeakErrors.length,
    startIndex: peakStartIdx,
...
// ── Analysis ──
const totalBets = totalCorrect + totalWrong;
const accuracy = totalBets > 0 ? ((totalCorrect / totalBets) * 100).toFixed(1) : 0;

// Peak distribution
const peakDist = { low: 0, medium: 0, high: 0 };
peakHistory.forEach(p => {
  if (p.height <= 3) peakDist.low++;
  else if (p.height <= 6) peakDist.medium++;
  else peakDist.high++;
});

// Martingala calculation (1→2→4, max 3 steps)
let bankroll = 0;
const betSizes = [1, 2, 4];
peakHistory.forEach(p => {
  if (p.height <= 3) {
    bankroll += 1; // Win on steps 1-3 gives +1
  } else {
    bankroll -= 7; // Lose all 3 steps = -1-2-4 = -7
  }
});

console.log(`RESULTADOS GENERALES:`);
console.log(`  Total predicciones: ${totalBets}`);
console.log(`  Correctas: ${totalCorrect} (${accuracy}%)`);
console.log(`  Incorrectas: ${totalWrong}`);
console.log(`\n`);

console.log(`DISTRIBUCIÓN DE PICOS:`);
console.log(`  Bajos (1-3):   ${peakDist.low} (${(peakDist.low/peakHistory.length*100).toFixed(1)}%)`);
console.log(`  Medios (4-6):  ${peakDist.medium} (${(peakDist.medium/peakHistory.length*100).toFixed(1)}%)`);
console.log(`  Altos (7+):    ${peakDist.high} (${(peakDist.high/peakHistory.length*100).toFixed(1)}%)`);
console.log(`  Total picos:   ${peakHistory.length}`);
console.log(`\n`);

console.log(`BANKROLL MARTINGALA (1→2→4):`);
console.log(`  Resultado: ${bankroll >= 0 ? '+' : ''}${bankroll} unidades`);
console.log(`\n`);

console.log(`ACCURACY POR MODO:`);
for (const [mode, stats] of Object.entries(modeStats)) {
  const t = stats.correct + stats.wrong;
  const acc = t > 0 ? ((stats.correct / t) * 100).toFixed(1) : 'N/A';
  console.log(`  ${mode}: ${stats.correct}/${t} = ${acc}%`);
}
console.log(`\n`);

// ── Show worst peaks (height >= 4) ──
const worstPeaks = peakHistory.filter(p => p.height >= 4).sort((a, b) => b.height - a.height);
console.log(`PICOS ALTOS/MEDIOS (≥4): ${worstPeaks.length} picos`);
console.log(`${'─'.repeat(70)}`);

worstPeaks.slice(0, 30).forEach((p, idx) => {
  const errors = [];
  for (let j = 0; j < p.height && j < p.predicted.length; j++) {
    errors.push(`#${j+1} pred=${p.predicted[j][0].toUpperCase()} real=${p.actual[j][0].toUpperCase()} mode=${p.modes[j]} (${p.colors[j]})`);
  }
  console.log(`\n  PEAK ${p.height} — posición ${p.startIndex}:`);
...
console.log(`ACCURACY POR LONGITUD DE RACHA:`);
console.log(`${'─'.repeat(50)}`);
for (const [streak, stats] of Object.entries(streakStats).sort((a, b) => parseInt(a[0]) - parseInt(b[0]))) {
  const t = stats.correct + stats.wrong;
  const acc = t > 0 ? ((stats.correct / t) * 100).toFixed(1) : 'N/A';
  console.log(`  Streak ${streak}: ${stats.correct}/${t} = ${acc}% (${stats.mode})`);
}

// ── Peak height distribution detail ──
console.log(`\nDISTRIBUCIÓN DETALLADA DE PICOS:`);
const heightCount = {};
peakHistory.forEach(p => { heightCount[p.height] = (heightCount[p.height] || 0) + 1; });
for (const [h, count] of Object.entries(heightCount).sort((a, b) => parseInt(a[0]) - parseInt(b[0]))) {
  const bar = '█'.repeat(Math.min(count, 50));
  console.log(`  Pico ${h}: ${count.toString().padStart(4)} ${bar}`);
}
