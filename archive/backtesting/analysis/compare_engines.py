#!/usr/bin/env python3
"""
Compare v4.5 vs v4.6 on the FULL 4,098-number sequence.
Show specific examples of long streaks and how each engine would behave.
"""
...
# ===== NEW v4.6.1 BREAK PROBS from THIS dataset =====
break_data = {}
...
NEW_PROBS = {}
for sl, d in break_data.items():
    total = d["break"] + d["continue"]
    if total >= 20:  # Only use if enough data
        NEW_PROBS[sl] = d["break"] / total * 100

print("=" * 70)
print("NUEVAS PROBABILIDADES DE ROTURA (4,098 spins)")
print("=" * 70)
print(f"\n{'Streak':>6} | {'Cases':>5} | {'Break%':>7} | {'v4.5/6':>8} | {'Diff':>7} | {'Nuevo Umbral'}")
print("-" * 70)

OLD = {2: 49.7, 3: 51.8, 4: 51.4, 5: 54.9, 6: 48.5, 7: 45.3, 8: 44.8, 9: 37.5}

for sl in sorted(NEW_PROBS.keys()):
    d = break_data[sl]
    total = d["break"] + d["continue"]
    bp = NEW_PROBS[sl]
    old = OLD.get(sl, 50.0)
    ...
    if bp >= 50:
        ...
        umb = f"SAME ({edge:.1f}%)"
    print(f"  {sl:>4}  | {total:>5} | {bp:>6.1f}% | {old:>7.1f}% | {diff:>+6.1f}pp | {umb}")

# Simulate v4.5 vs v4.6 vs v4.6.1 (with new probs)
print("\n" + "=" * 70)
print("v4.5 vs v4.6 vs v4.6.1 (con nuevas probs)")
print("=" * 70)

v45_ok = 0; v45_tot = 0
v46_ok = 0; v46_tot = 0
v461_ok = 0; v461_tot = 0

v45_wrong3 = 0; v45_wrong4 = 0
v46_wrong5 = 0
...
for i in range(1, len(nz_colors)):
    ...
    old_bp = OLD.get(streak_len, 50.0) if streak_len <= 9 else 37.5
    new_bp = NEW_PROBS.get(streak_len, 50.0)
    
    # v4.5: pushes opposite at streak 3+
    if streak_len >= 3:
        v45_tot += 1
        if streak_len >= 5:
            if old_bp >= 50:
                if actual == opp: v45_ok += 1
            else:
                if actual == sc: v45_ok += 1
        else:
            if actual == opp:
                v45_ok += 1
            else:
                if streak_len == 3: v45_wrong3 += 1
                elif streak_len == 4: v45_wrong4 += 1
    
    # v4.6: pushes opposite only at streak 5+
    if streak_len >= 5:
        v46_tot += 1
        if old_bp >= 50:
            if actual == opp: v46_ok += 1
            else: v46_wrong5 += 1
        else:
            if actual == sc: v46_ok += 1
    
    # v4.6.1: uses NEW break probs
    if streak_len >= 5:
        v461_tot += 1
        if new_bp >= 50:
            if actual == opp: v461_ok += 1
        else:
            if actual == sc: v461_ok += 1

print(f"\nv4.5 (streaks 3+):    {v45_ok}/{v45_tot} correct ({v45_ok/max(1,v45_tot)*100:.1f}%)")
print(f"  Wrong at streak 3: {v45_wrong3}")
print(f"  Wrong at streak 4: {v45_wrong4}")
print(f"\nv4.6 (streaks 5+):    {v46_ok}/{v46_tot} correct ({v46_ok/max(1,v46_tot)*100:.1f}%)")
print(f"  Wrong at streak 5: {v46_wrong5}")
print(f"\nv4.6.1 (new probs):   {v461_ok}/{v461_tot} correct ({v461_ok/max(1,v461_tot)*100:.1f}%)")

# What should the new threshold be?
print("\n" + "=" * 70)
print("RECOMENDACION: Nuevo umbral de activacion")
print("=" * 70)
print("""
DATOS DE ESTA SECUENCIA (4,098 spins):
  Streak 2:  51.4% rompe → neutro (1.4pp de ventaja)
  Streak 3:  51.1% rompe → neutro (1.1pp de ventaja)
  Streak 4:  54.4% rompe → OPUESTO (4.4pp de ventaja!)
  Streak 5:  48.6% rompe → MISMO COLOR (1.4pp para continuar)
  Streak 6:  44.6% rompe → MISMO COLOR (5.4pp para continuar!)
  Streak 7:  45.2% rompe → MISMO COLOR (4.8pp para continuar)
  Streak 8:  35.3% rompe → MISMO COLOR (14.7pp para continuar!!)

DESCUBRIMIENTO IMPORTANTE:
  - Streak 4 tiene la MEJOR ventaja para opuesto (54.4%)
  - Streak 5 NO tiene ventaja para opuesto (48.6% < 50%)
  - Streak 8 es el punto mas fuerte para mismo color (35.3%)

  El v4.6 actual activa anti-racha en streak 5 (54.9% hardcoded)
  pero en ESTA secuencia streak 5 solo tiene 48.6%!
  
RECOMENDACION v4.6.1:
  - Streak 2-3: Neutral (Markov decide)
  - Streak 4: Empujar OPUESTO (54.4% = ventaja REAL de 4.4%)
  - Streak 5: Neutral (48.6% = sin ventaja)
  - Streak 6: Empujar MISMO COLOR (44.6% = ventaja de 5.4%)
  - Streak 7+: Empujar MISMO COLOR (racha continúa)
""")
