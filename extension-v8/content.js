(function() {
  ...
  function _rs() {
    ...
    if (_ka.v) { l.push('RECUPERANDO... (#' + _ka.x + ')'); }
    else {
    ...
    if (_ci2) {
      _ci2.style.background = _ka.v ? '#f59e0b' : (!_ka.a || _ka.r === 401 || _ka.r === 403) ? '#ef4444' : '#22c55e';
      _ci2.style.boxShadow = _ci2.style.background.replace(')', ', 0 0 6px ' + _ci2.style.background + ')').replace('rgb', 'rgba');
    }
  }
  ...
  function _mkUI() {
    ...
    p.appendChild(_st); p.appendChild(lr);
    ...
    c.appendChild(p); c.appendChild(b); document.body.appendChild(c);
    _ci = document.getElementById('x-n');
    _ci2 = document.getElementById('x-d2');
  }
